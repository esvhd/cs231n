...
class PretrainedCNN(object):
    ...
    def __init__(self, dtype=np.float32, num_classes=100, input_size=64, h5_file=None):
        ...
        self.bn_params.append({'mode': 'train'})
        self.params['W%d' % (i + 3)] = np.sqrt(2.0 / hidden_dim) * \
    ...
    def forward(self, X, start=None, end=None, mode='test'):
        ...
        for i in range(start, end + 1):
            ...
            if 0 <= i < len(self.conv_params):
                ...
                next_a, cache = conv_bn_relu_forward(
                    prev_a, w, b, gamma, beta, conv_param, bn_param)
                ...
                bn_param = self.bn_params[i]
                bn_param['mode'] = mode
                next_a, cache = affine_bn_relu_forward(
                    prev_a, w, b, gamma, beta, bn_param)
    ...
    def loss(self, X, y=None):
        ...
        """
        # Note that we implement this by just caling self.forward and
        # self.backward
